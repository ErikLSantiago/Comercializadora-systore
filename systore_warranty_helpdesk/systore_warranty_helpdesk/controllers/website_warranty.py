...
class WarrantyWebsiteController(http.Controller):
    def _verify_recaptcha_v3(self, action_expected="warranty_request"):
        """Verify Google reCAPTCHA v3 token sent from the form.

        Uses Odoo's configured keys (Settings > Integrations > reCAPTCHA).
        If keys are not configured, returns True to avoid blocking (admin should configure keys).
        """
        icp = request.env["ir.config_parameter"].sudo()
        secret_key = icp.get_param("recaptcha_private_key")
        site_key = icp.get_param("recaptcha_public_key")
        if not secret_key or not site_key:
            return True, None  # not configured

        token = request.params.get("recaptcha_token_response") or request.params.get("g-recaptcha-response")
        if not token:
        ...
        # v3 score/action checks (best-effort)
        try:
            min_score = float(icp.get_param("recaptcha_minimum_score", "0.7"))
        except Exception:
            min_score = 0.7
        ...
        if action_expected and action and action != action_expected:
    ...
    def warranty_request(self, **post):
        ...
        if request.httprequest.method == "POST":
            # Verificar reCAPTCHA (v3) para evitar spam/bots
            ok_recaptcha, _reason = self._verify_recaptcha_v3(action_expected="warranty_request")
            if not ok_recaptcha:
                qcontext = {"error_recaptcha": True}
                return request.render("systore_warranty_helpdesk.warranty_request_template", qcontext)
