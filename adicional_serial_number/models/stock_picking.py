...
# --- Totales de demanda vs capturados para colorear el smart button ---
serial_demand_total = fields.Integer(string="Demanda (seriales)", compute="_compute_serial_totals", store=False)
serial_captured_count = fields.Integer(compute="_compute_serial_totals", string="Seriales capturados", store=False)
...
def _compute_serial_totals(self):
    ...
    for p in self:
        # demanda = suma de cantidades demandadas para productos con tracking 'serial'
        demand = 0
        for m in p.move_ids_without_package:
            if m.product_id and m.product_id.tracking == 'serial':
                # asumimos 1 serial por unidad
